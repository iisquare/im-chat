...
import com.iisquare.im.server.broker.ServerHandler;
...
public class GateController extends WebController {
    ...
    public String routeAction() {
        ...
        route.put("ws", "ws://" + node + ":" + config.getHttpPort() + ServerHandler.WEB_SOCKET);
        route.putObject("comet")
            .put("push", "http://" + node + ":" + config.getHttpPort() + ServerHandler.COMET_PUSH)
            .put("pull", "http://" + node + ":" + config.getHttpPort() + ServerHandler.COMET_PULL);
